#if defined(ARDUINO_GENERIC_G030K8TX)

#include "RBDmcuSTM32G030K8TX.h"
#define __cplusplus
...
dimmerLamp::dimmerLamp(int user_dimmer_pin, int zc_dimmer_pin)
    : dimmer_pin(user_dimmer_pin), zc_pin(zc_dimmer_pin) {
    current_dim++;
...
//TODO: determine if TIM1/TIM2 matters, any other alts, if not then document
void dimmerLamp::timer_init(void) {
    ...
    MyTim->setOverflow(95, MICROSEC_FORMAT);
...
bool dimmerLamp::getState(void) {
    return dimState[this->current_num] == ON;
}

void dimmerLamp::changeState(void) {
    dimState[this->current_num] = (dimState[this->current_num] == ON) ? OFF : ON;
}
...
int dimmerLamp::getMyCounter(void) {
    return myCounter;
}
